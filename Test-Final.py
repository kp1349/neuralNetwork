...
class NeuralNetwork:
	...
	def Test_debug(self, input_data, output_data): #josh
		...
		lines = []
		for i in range(len(input_data)):
			lines.append(str(output_data[i])+" --> "+str(self.Forward(input_data[i])))
			sum = 0.0
			for j in range(len(output_data[i])):
				temp = abs(output_data[i][j] - self.Forward(input_data[i])[j])
				sum += temp
				if (temp>max_value):
					max_value = temp
					max_ndx = i + 1
...
with open('iris dataset/iris.train') as file:
...
with open('iris dataset/iris.test') as file:
	lines = file.readlines()
